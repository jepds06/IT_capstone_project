  ...
  <div class="p-6">
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg">
        ...
        <div v-if="isCompleted" class="flex justify-end font-bold m-2">
          <span>Total:{{
            new Intl.NumberFormat("en-PH", {
              style: "currency",
              currency: "PHP",
            }).format(
              materialQuotation.reduce(
                (sum, item) =>
                  parseFloat(sum) + parseFloat(item.qtyNeeded * item.quotePrice),
                0
              )
            )
          }}</span>
...
const transformationDataForQuotation = async (quotation) =>{
  ...
  const materialTransform = materialsList.value
    ?.map((material, index) => {
      let productMaterial = result?.quotation_details.find(
        (detail) =>
          detail.production_material.product_material.material.materialID ===
      ...
      const qtyNeeded = materials.value
        ?.filter((value) => value.materialID === material.materialID)
        ?.reduce((sum, item) => parseInt(sum) + parseInt(item.qtyNeeded), 0);
      if (productMaterial) {
        return {
          materialID: material.materialID,
    ...
    .filter((value) => value.qtyNeeded !== 0)?.map((v, index) => {
...
const completeQuotation = async () => {
  ...
  const result = await apiService.put(
    `/api/quotations/${selectedQuotation.value.quoteID}`,
    {
  ...
  fetchQuotations();
  showSuccessQuotationCompleteMessage("Quotation successfully completed");
};
...
const saveQuotationDetail = async () => {
  if (!quotationDetailForm.value?.quotePrice) {
    await Promise.all(
      materialDetail.value?.map(async (value) => {
        if (value.qteDetailID) {
          return await apiService.delete(
            `/api/quotationDetails/${value.qteDetailID}`
          );
        }
        return value;
      })
...
const fetchQuotations = async () => {
  ...
  quotations.value = result.data
    ?.filter((value) =>
      ...
      value.isCompleted == props.isCompleted
        ? 1
        : 0
    )
    .map((value, index) => {
...
const fetchMaterials = async () => {
  const result = await apiService.get("/api/materials");
