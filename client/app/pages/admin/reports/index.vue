    ...
    <div class="m-8 space-y-6">
      ...
      <div class="flex gap-4">
        <input
          ...
          class="px-4 py-2 w-1/4 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <input
          type="date"
          v-model="startDate"
        ...
        <input
          ...
          class="px-4 py-2 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button @click="applyFilters" class="px-6 py-2 bg-blue-600 text-white rounded shadow hover:bg-blue-700">
          Generate Report
        </button>
      </div>
      ...
      <div class="flex justify-end mb-4">
        <button @click="exportPDF" class="px-4 py-2 bg-red-600 text-white rounded shadow hover:bg-red-700 mr-2">
          Export to PDF
        </button>
        <button @click="exportCSV" class="px-4 py-2 bg-green-600 text-white rounded shadow hover:bg-green-700">
          Export to CSV
        </button>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow overflow-hidden">
          ...
          <tbody>
            <tr v-for="report in filteredReports" :key="report.orderID" class="border-b hover:bg-gray-100">
              <td class="px-6 py-3">{{ report.orderID }}</td>
              <td class="px-6 py-3">{{ report.customerName }}</td>
              <td class="px-6 py-3">{{ report.totalAmount }}</td>
              <td class="px-6 py-3">{{ report.orderDate }}</td>
              <td class="px-6 py-3">
                <i :class="getStatusIcon(report.status)" :title="report.status"></i>
              </td>
              <td class="px-6 py-3 flex gap-3">
                <button @click="viewReport(report.orderID)" class="text-blue-500 hover:text-blue-700">
                  <i class="fa-solid fa-eye"></i>
                </button>
                <button @click="editReport(report.orderID)" class="text-yellow-500 hover:text-yellow-700">
      ...
      <div v-if="isViewModalOpen" class="modal-overlay">
        <div class="modal-content">
          ...
          <button @click="closeModal" class="mt-4 bg-blue-600 text-white px-4 py-2 rounded">Close</button>
        </div>
      </div>
      ...
      <div v-if="isEditModalOpen" class="modal-overlay">
        <div class="modal-content">
          ...
          <label>
            Total Amount:
            <input v-model="modalData.totalAmount" class="mt-2 p-2 border rounded" type="text" />
          </label>
          <button @click="saveChanges" class="mt-4 bg-green-600 text-white px-4 py-2 rounded">Save</button>
          <button @click="closeModal" class="mt-4 bg-gray-600 text-white px-4 py-2 rounded">Cancel</button>
        </div>
      </div>
    </div>
  </template>
...
const exportCSV = () => {
  const headers = ["Order ID", "Customer Name", "Total Amount", "Order Date", "Status"];
  const csvContent = [
    headers.join(","), // CSV headers
    ...reports.value.slice(0, 5).map(report => [
      report.orderID,
      report.customerName,
      report.totalAmount,
      report.orderDate,
  ...
  const filteredReports = computed(() => {
    return reports.value
      .filter(
        (report) =>
          report.orderID.toString().includes(searchQuery.value) ||
          report.customerName.toLowerCase().includes(searchQuery.value.toLowerCase())
      )
      .slice(start.value, end.value);
  });
  
  const start = computed(() => (page.value - 1) * itemsPerPage);
  const end = computed(() => Math.min(page.value * itemsPerPage, reports.value.length));
