<template>
  <div class="m-8">
    ...
    <div class="flex justify-between mb-4">
      <h1 class="text-2xl font-extrabold">Materials</h1>
      <!-- <button
        @click="openForm('add')"
      ...
      <UButton icon="material-symbols:add-2-rounded" size="md" color="primary" variant="solid" label="Material"
        title="Add Material" @click="openForm('add')" :trailing="false" />
    </div>

    <!-- Modal for Add/Edit Form -->
    <Modal :isVisible="isFormVisible" title="Material Form" :showSave="true" @update:isVisible="isFormVisible = $event"
      @save="confirmSave">
      <template v-slot:body>
        <form @submit.prevent="saveMaterial">
          <div class="mb-4" v-if="formMode === 'edit'">
            <label for="id" class="block text-sm font-medium text-gray-700">Id</label>
            <input v-model="form.materialID" type="text" id="id"
          ...
          <div class="mb-4">
            ...
            <textarea v-model="form.specification" id="specs" rows="3"
          ...
          <div class="mb-4">
            ...
            <input v-model="form.brand" type="text" id="description"
              class="mt-1 block w-full border border-gray-300 rounded-lg p-2" />
          </div>
          <div class="mb-4">
            <label for="unitOfMeasure" class="block text-sm font-medium text-gray-700">Unit of Measure</label>
            <input v-model="form.unitOfMeasure" id="unitPrice"
              class="mt-1 block w-full border border-gray-300 rounded-lg p-2" />
          </div>
          <!-- <div class="mb-4">
    ...
    <div v-if="showConfirmationModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
      @keydown.enter="saveMaterial" tabindex="0">
