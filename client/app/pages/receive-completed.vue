    ...
    <div class="p-6 bg-gray-50 min-h-screen">
      ...
      <div v-if="activeTab === 'To Receive'">
        <div
          ...
          :key="order.storeName"
          class="bg-gradient-to-r from-white to-gray-50 rounded-xl shadow-lg p-6 mb-6 transition-transform duration-300 hover:scale-105"
        >
          <div class="flex justify-between items-center border-b pb-3">
            <h2 class="text-2xl font-bold text-gray-800">{{ order.storeName }}</h2>
          ...
          <div class="flex items-start gap-6 mt-4">
            ...
            <div>
              ...
              <p class="mt-2 text-lg text-gray-500">
                Quantity:
                <span class="font-medium text-gray-700">{{ order.product.quantity }}</span>
              </p>
              <p class="mt-1 text-lg text-gray-500">
                Price:
                <span class="font-medium text-gray-700">₱{{ order.product.price.toFixed(2) }}</span>
              </p>
          ...
          <div class="mt-6">
            <p class="text-lg text-gray-500">
              Expected Delivery:
              <span class="font-medium text-gray-700">{{ order.expectedDelivery }}</span>
            </p>
            <p class="mt-1 text-lg text-gray-500">
              Status:
              <span class="font-medium text-gray-700">{{ order.deliveryStatus }}</span>
            </p>
          </div>
      ...
      <div v-if="activeTab === 'Completed'">
        ...
        >
          <div class="flex justify-between items-center border-b pb-3">
            <h2 class="text-2xl font-bold text-gray-800">{{ order.storeName }}</h2>
            <span class="text-lg font-semibold uppercase text-green-500">
              {{ order.status }}
            </span>
          </div>
